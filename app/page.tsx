...
export default function Home() {
  ...
  const calculate = () => {
    ...
    if (!isNaN(num)) {
      ...
      const estimatedMoney = num * 50 * 365;
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-white/10 backdrop-blur-lg border border-white/10 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <input
            ...
            className="w-full p-4 rounded-xl bg-white text-black mb-4 outline-none focus:ring-2 focus:ring-white/50"
          />
          ...
          <div className="flex flex-wrap gap-2 justify-center mb-6">
            <button onClick={() => setValue("2")} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full text-sm">
              📱 Instagram
            </button>
            <button onClick={() => setValue("1")} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full text-sm">
              ☕ Kahve
            </button>
            <button onClick={() => setValue("3")} className="bg-white/10 hover:bg-white/20 px-4 py-2 rounded-full text-sm">
              🎬 Dizi
            </button>
          </div>
            ...
            <div className="mt-10 space-y-5">
              ...
              {money && (
                <p className="text-green-400 font-semibold">
                  Yaklaşık {money.toLocaleString()} TL harcıyorsun
                </p>
              )}
              ...
              {age && (
                <p className="text-purple-400">
                  Eğer bu şekilde devam edersen, 60 yaşına kadar{" "}
                  {days * (60 - Number(age))} gün kaybedeceksin.
                </p>
              )}
              ...
              <button
                onClick={() => {
                  ...
                  alert("Kopyalandı! Paylaş 🚀");
                }}
                className="mt-4 bg-red-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-red-600 transition"
              >
                Paylaş
              </button>
